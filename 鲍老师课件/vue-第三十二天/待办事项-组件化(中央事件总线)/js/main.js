...
const ListItem = {
  ...
  methods: {
    edit() {
      ...
      bus.$emit('edit-item', this.getItem);
    },
    cancelEdit() {
      console.log('cancelEdit');
      bus.$emit('cancel-edit-item', this.getItem);
    },
    del() {
      console.log('del');
      bus.$emit('del-item', this.getItem);
    },
    save() {
      bus.$emit('save-item', {
        item: this.getItem,
...
const ListContainer = {
  ...
  template: `<div>
               <ul v-if='getList.length' class='descul'>
                 <ListItem :getItem='item' :key='item.id' v-for='item of getList' v-show='showFilter.allShow || (showFilter.partShow?item.isFinish:!item.isFinish)'></ListItem>
               </ul>
               <p v-else>没有数据！！！</p>
             </div>`,
  components: {
...
const vm = new Vue({
  ...
  created() {
    ...
    bus.$on('del-item', item => {
      console.log(item);
      const index = this.list.indexOf(item);
      this.list.splice(index, 1);
    });

    bus.$on('cancel-edit-item', item => {
      console.log(item);
      const index = this.list.indexOf(item);
      this.list.splice(index, 1, {
        title: item.title,
